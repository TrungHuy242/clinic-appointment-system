...
export default function LookupPage() {
  ...
  async function handleLookupInternal(searchCode, searchPhone) {
    setLoading(true);
    setResult(null);
    setResults([]);
    setHasSearched(true);
...
useEffect(() => {
  ...
  const handleBack = () => {
    ...
    navigate(`/patient/appointments?tab=${tab}`);
  };
    ...
    <div className="lk-page">
      ...
      {!cameFromPatientAppointments && (
        <div className="lk-header">
          ...
          <p>Nhập số điện thoại đã đăng ký để xem thông tin lịch hẹn</p>
        </div>
      )}
      ...
      {!cameFromPatientAppointments && (
        <form onSubmit={handleLookup} className="lk-search">
          ...
          <div className="lk-field">
            <label>Mã lịch hẹn (tuỳ chọn)</label>
            <div className="lk-input-wrap">
              <QrCode size={18} />
              <input
                type="text"
      ...
      {!loading && result === "not_found" && (
        <div className="lk-result lk-result--error">
          ...
          <p>Không có lịch hẹn nào phù hợp với thông tin bạn đã nhập. Vui lòng kiểm tra lại số điện thoại và mã lịch hẹn.</p>
        </div>
      )}
          ...
          <div className={`lk-banner ${statusInfo?.className || ""}`}>
            ...
            <div className="lk-banner-content">
              ...
              <p>
                ...
                {isPending && "Vui lòng xác nhận để giữ khung giờ đã chọn."}
                {result.status === "CANCELLED" && "Lịch hẹn đã bị hủy. Bạn có thể đặt lại."}
                {result.status === "COMPLETED" && "Lịch hẹn đã hoàn thành. Cảm ơn bạn."}
              </p>
            </div>
            {isPending && (
              ...
              <div className="lk-info-items">
                ...
                <div className="lk-info-item">
                  ...
                  <div>
                    ...
                    <span className="lk-info-sub">123 Nguyễn Văn Linh, Đà Nẵng</span>
                  </div>
                </div>
              </div>
    ...
    ✅ Hủy lịch hẹn thành công!
      ...
      {!cameFromPatientAppointments && !hasSearched && !loading && !result && results.length === 0 && (
        <div className="lk-help">
          <Search size={20} />
          <p>Nhập số điện thoại đã dùng khi đặt lịch để tra cứu. Bạn cũng có thể nhập thêm mã lịch hẹn để tìm chính xác hơn.</p>
        </div>
      )}
