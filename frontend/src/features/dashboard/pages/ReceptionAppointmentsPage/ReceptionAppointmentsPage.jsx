...
const STATUS_CONFIG = {
  CONFIRMED: { label: "Äã xác nhận", variant: "success" },
  PENDING_PA1: { label: "Chờ xác nhận", variant: "warning" },
  CHECKED_IN: { label: "Äã check-in", variant: "info" },
  CANCELLED: { label: "Äã hủy", variant: "danger" },
  ...
  },
  {
    ...
    render: (row) => (
      <div className="reception-appointments__actions">
        <button className="dash-action-btn dash-action-btn--sm" title="Chi tiết" type="button">
          <Search size={16} />
        </button>
...
export default function ReceptionAppointmentsPage() {
  ...
  const filtered = appointments.filter((appointment) => {
    const matchSearch =
      ...
      appointment.code?.includes(search);
    ...
    <div className="dash-page reception-appointments">
      <div className="dash-page-header">
        <div>
          ...
          <p className="dash-page-sub">Theo dõi và cập nhật trạng thái lịch hẹn trong ngày</p>
        </div>
      ...
      <div className="dash-stats-row">
        {STAT_CARDS.map((card) => (
          ...
          >
            <div className="dash-stat-icon reception-appointments__stat-text">{React.createElement(card.icon, { className: "mc-icon mc-icon--md" })}</div>
