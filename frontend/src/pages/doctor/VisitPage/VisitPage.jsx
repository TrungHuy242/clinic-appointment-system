...
export default function VisitPage() {
  ...
  useEffect(() => {
    async function loadQueue() {
      try {
        const data = await getVisitQueue();
        setQueue(Array.isArray(data) ? data : []);
      } catch (error) {
        console.error("Failed to load visit queue:", error);
        setQueue([]);
        showToast("Không tải được hàng chờ khám.", "danger");
      }
    }
  ...
  const loadDetail = useCallback(async (code) => {
    ...
    try {
      ...
      if (result.draft) {
        ...
        setPrescription(
          Array.isArray(result.draft.prescription) && result.draft.prescription.length > 0
            ? result.draft.prescription
            : [emptyRx()]
        );
      } else {
        setDiagnosis("");
      ...
      setDetail(null);
      showToast("Không tải được phiếu khám.", "danger");
    } finally {
  ...
  async function handleSaveDraft() {
    if (!activeCode) {
      showToast("Chưa chọn phiếu khám.", "warning");
      return;
    }
    ...
    try {
      ...
      showToast(error.message || "Lưu nháp thất bại.", "danger");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleComplete() {
    ...
    try {
      await completeVisit(activeCode, {
        diagnosis: diagnosis.trim(),
        notes: notes.trim(),
        prescription,
      });
      ...
      await loadDetail(activeCode);
    } catch (error) {
      console.error("Complete visit failed:", error);
      showToast(error.message || "Hoàn tất thất bại.", "danger");
    } finally {
      setCompleting(false);
    }
  }
        ...
        {detail && !loadingDetail && (
          <>
            ...
            <div className="vt-action-bar">
              <Button variant="secondary" onClick={handleSaveDraft} disabled={saving || completing}>
