...
class ReceptionCheckinLookupAPIView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        return Response({
            'state': state,
            'message': 'Check-in thành công' if state == "VALID" else f"Không thể check-in: {state}",
            'appointment': AppointmentSerializer(appointment).data
        })
class ReceptionMoveToWaitingAPIView(APIView):#Chuyển sang bác sĩ
    def patch(self, request, lookup_value, *args, **kwargs):
        try:
            appointment = get_appointment_by_id_or_code(lookup_value)
        except Exception:
        ...
        # Chỉ cho phép chuyển khi đã check-in
        if appointment.status != AppointmentStatus.CHECKED_IN:
